...
import DisplayStatefulWidgetIcon from "../DisplayStatefulWidgetIcon";
...
beforeEach(() => {
  ...
  // Default: recoloring succeeds
  mockLoadAndRecolorSvg.mockResolvedValue(
    `<svg data-testid="recolored-svg"><circle cx="20" cy="20" r="10" fill="#ff0000" /></svg>`
  );
});
...
describe("DisplayStatefulWidgetIcon - Basic Tests", () => {
  ...
  it("uses no_icon fallback when all formats fail", async () => {
    ...
    mockLoadAndRecolorSvg.mockRejectedValueOnce(new Error("All failed"));
    mockGetPrimaryIconPath.mockReturnValue("/icons/stateful/no_icon.svg");

    renderWithKey(makeProps({ iconName: "unknown" }));

    await waitFor(() => {
      const img = screen.getByAltText(/unknown/i) as HTMLImageElement;
      expect(img).toBeInTheDocument();
