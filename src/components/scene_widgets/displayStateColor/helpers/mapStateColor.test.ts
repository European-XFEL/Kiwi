import { mapGuiStateColor } from "./mapStateColor";
...
describe('mapGuiStateColor', () => {
  ...
  it('maps CHANGING family', () => {
    const cases = ['CHANGING', 'INCREASING', 'DECREASING', 'STARTING', 'HEATING', 'SWITCHING_OFF'];
  ...
  it('maps ACTIVE family', () => {
    const cases = ['ACTIVE', 'ON', 'OPENED', 'STARTED', 'LOCKED', 'MONITORING', 'INTERLOCK_OK'];
    for (const s of cases) {
  ...
  it('maps PASSIVE family', () => {
    const cases = ['PASSIVE', 'OFF', 'CLOSED', 'STOPPED', 'UNLOCKED', 'DISENGAGED', 'IGNORING'];
    for (const s of cases) {
