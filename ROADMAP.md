...
- [ ] Support the `DisplayTrendGraph` widget (MVP-mode: most important features; not necessarily all) (https://git.xfel.eu/karaboweb/kiwi/-/issues/11)
...
- [ ] Support scene embedding by arbitrary webpages. More info on iframe embedding (see https://blog.logrocket.com/best-practices-react-iframes/#best-practices-using-iframes-react). A no-chrome mode must be added to Kiwi along with a parameter that allows specifying OBSERVER mode that will bypass authentication. Also the scene should be loaded with a zoom factor that uses as much space of the iframe client area while keeping its aspect ratio (actually there should be a new URL parameter, ZoomFactor with range [0.0, 20.0] (20 is an initial max suggestion) with 0.0 "hijacked" to mean best fit; 1.0 is the default). For supporting EXPERT and OPERATOR modes, either the refresh-token can be used (after confirming the browser’s LocalStorage will be properly populated, since the iframe is embedded in an arbitrary page) or the login panel will be activated. Btw, as Kiwi ends up being a GUI client the login should use an "extended" Karabo Authentication Server that integrates with KeyCloak. Proof-of-concept suggestions: a vanilla web page with the SceneCanvas as an iframe and a Python Notebook with an embedded SceneCanvas as an iframe (attention to https/http issues as noted at https://www.pythonanywhere.com/forums/topic/7362/)
